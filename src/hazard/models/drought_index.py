...
class DroughtIndicator:
    ...
    def _calculate_spei_chunk(self, chunk_name, data_chunks, ds_chunked: xr.Dataset, gcm, scenario):
        ...
        ds_spei_slice.to_zarr(store=store, mode='r+', 
                                region={"lat": slice(lat_indexes[0], lat_indexes[-1] + 1), "lon": slice(lon_indexes[0], lon_indexes[-1] + 1),"time": slice(time_indexes[0],time_indexes[-1]+1)}) #??? updated store and slice time
    ...
    def _calculate_spei_for_slice(
            ...
            *,
            ...
            scenario,
            num_workers = 2):
        ds_tas = self.read_quantity_from_s3_store(gcm, scenario, "tas", lat_min, lat_max, lon_min, lon_max).chunk({'time': 100000}).compute()
        ds_pr = self.read_quantity_from_s3_store(gcm, scenario, "pr", lat_min, lat_max, lon_min, lon_max).chunk({'time': 100000}).compute() 
    ...
    def calculate_annual_average_spei(self, gcm: str, scenario: str, central_year: int, target: OscZarr):
        ...
        spei_annual_all = xr.DataArray(spei_annual, coords={"spei_index": self.spei_threshold, "lat": lats_all, "lon": lons_all,}, dims=["spei_index", "lat", "lon"])
    ...
    def run_single(self, item: BatchItem, target: OscZarr , calculate_spei = True, calculate_average_spei = True,progress_store: Optional[ProgressStore] = None):
        if calculate_spei:
